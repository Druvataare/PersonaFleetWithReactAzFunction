...
**Still open:** the Fabric-side grant. `/api/health` now reaches Fabric and is refused: `Could not login because the authentication failed.` The managed identity obtained a token (a credential failure raises a different error) and Fabric rejected the principal, which is the expected state until the steps below are done.
