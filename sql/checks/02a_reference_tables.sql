...
SELECT * FROM dbo.dimpersona ORDER BY SortOrder;
...
FROM dbo.dimuser u
LEFT JOIN dbo.dimpersona p ON p.SourcePersonaValue = u.Persona
...
LEFT JOIN dbo.dimcpumodel c ON c.CPUModel = d.CPUModel
GROUP BY d.CPUModel, c.CpuScore
ORDER BY c.CpuScore;

-- 4 · Four SLA targets: P1 4 · P2 8 · P3 72 · P4 120 hours
SELECT * FROM dbo.dimslatarget ORDER BY Priority;
