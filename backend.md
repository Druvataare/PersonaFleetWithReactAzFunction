...
| 2e   | `persona_vw_api_v1_*` views; calibrate boot, crash, free-space and battery baselines             | [calibration](sql/checks/02e_calibration.sql) · [sql/gold/persona_vw_api_v1.sql](sql/gold/persona_vw_api_v1.sql) · [check](sql/checks/02e_views.sql) | Done 18 Sep 2026; view row counts and end-to-end device check deferred |
...
**Deferred (18 Sep 2026), to pick up later.** (1) Results 2 and 4 of [sql/checks/02e_views.sql](sql/checks/02e_views.sql): row counts per view — `device_ticket` about 1,900, `device_app` 95,000 — and one device end to end. The views were created and their column lists verified. (2) Part 2f: schedule `notebooks/02d_gold_tables.py` daily through a Data Factory pipeline, with a run log that drives the freshness label. Until then the gold tables are refreshed by running the notebook by hand, and `persona_vw_api_v1_migration` stays empty because only one persona snapshot exists.
