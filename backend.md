...
**Two deploy-time fixes (22 Sep 2026).** Static Web Apps first rejected the config — "A route is covered up by a wildcard route" — because it matches routes top to bottom, not by specificity; `/*` had to move after the specific routes. Once deployed, `/` was still reachable with no sign-in: `navigationFallback` rewrites every unmatched path to `/index.html`, and role checks apply to that _rewritten_ path, so it matched the unprotected `/index.html` route instead of falling through to `/*`. Verified with `curl -D -`: `/api/health` correctly 302'd to `/.auth/login/aad`, `/` returned 200. Fixed by adding `allowedRoles: ["authenticated"]` to the `/index.html` route directly, since that is what every SPA page resolves to. Confirmed working end to end: `/`, `/change` and other SPA routes now 302 to `/.auth/login/aad`; static assets stay public; the full CI pipeline (lint, 533 tests, smoke, audit) stayed green on the same deploy.

**Admin consent required (22 Sep 2026).** First real sign-in attempt showed "Approval required" — this tenant has user consent disabled, so a new app registration's permissions (`User.Read`, `offline_access` — the portal's default grant, nothing added by us) need an admin to approve once for everyone, not a per-user prompt. Fix: **Entra admin center → App registrations → `PersonaFleetCommand` → API permissions → Grant admin consent for hcldwplabs.com**, by a Global/Application/Cloud Application Administrator.
...
**Why `persona_policy` exists here and not in the lakehouse.** These are the inputs to every number the portal shows: `fitClass` compares a device against the baseline, and the five weights combine the pillar scores into persona health. They are also the values people argue over, and the Baselines page exists specifically to edit them — so keeping them in the lakehouse would mean a pipeline run to change a single number. Each column is a threshold a device is measured against, except the five `Weight*` columns, which must total 100 or every score in the portal is silently rescaled.

The next three tables are **writeback**: the portal creates rows in them (AD-5). They have no home in the lakehouse at all, because its SQL analytics endpoint is read-only — a write there is not slow, it is impossible.

**`persona_change`** — one row per persona reassignment requested from the portal ("move this user from Knowledge Worker to Engineering"). It is the audit trail of who asked for what and when, and the source `/api/change/migrations` reads back. Columns: `ChangeId`, `UserId`, `DeviceId`, `FromPersonaKey`, `ToPersonaKey`, `RequestedBy` (taken from the caller's Entra token, never the request body — otherwise a client could attribute its own change to someone else), `RequestedUtc`, `IdempotencyKey` (unique, so a retried POST is recorded once).

**`persona_provisioning_request`** — one row per hardware request raised against an under-provisioned device; it produces the `REQ…` number and assignment group the portal shows the user after they click through. Columns: `RequestNumber` (generated by the database as `REQ` plus a zero-padded `IDENTITY` — the number is user-facing and must be unique, and two Function App instances cannot agree on the next one without a round trip), `DeviceId`, `AssignmentGroup`, `Status`, `RequestedBy`, `RequestedUtc`, `IdempotencyKey` (unique).

**`persona_app_exception`** — one row per request to run an application outside the persona's catalogue, carrying the approval workflow's state. Columns: `ExceptionId`, `UserId`, `PersonaKey`, `AppName`, `Reason`, `State` (`Pending` / `Approved` / `Rejected` — exactly the `ExceptionState` union, so a stored value cannot reach the front end and match no branch), `RaisedUtc`, `DecidedBy`, `DecidedUtc`. The last two are null only while `Pending`: an exception reading `Approved` with nobody accountable for approving it would defeat the point of keeping the record.
...
**Status:** Done, 22 Sep 2026. `/api/health` reports `status: ok` with a live Fabric connection through the Static Web App.

**Built while the Function App is blocked (22 Sep 2026).** The connection layer needs only configuration, so it is done and tested without Azure: `apps/api/src/fabric/` — `config.ts` (settings `FabricSqlEndpoint`, `FabricSqlDatabase`, optional timeouts; names exactly what is missing), `pool.ts` (one pool per process, token from `DefaultAzureCredential` — managed identity in Azure, `az login` locally — refreshed five minutes before expiry, concurrent cold starts share one connect), `query.ts` (the only place SQL runs: named, parameterised, retried up to twice on transient faults with a fresh connection, permanent faults such as a missing grant fail at once, every row validated with `zod` so a renamed column fails naming the field), `freshness.ts` (reads `persona_vw_api_v1_freshness`; the health check). `/api/health` now returns 503 with the reason when Fabric is unreachable. 13 new tests use a fake driver: retry, no-retry on permission errors, token refresh, shared cold start, shape errors, unconfigured, connected. Not yet run against real Fabric — that needs the endpoint address.

**Deployment packaging fixed (22 Sep 2026), once `func-personalfleet-api` existed and had no code.** The first bundling approach — esbuild bundling everything, including `@azure/functions`, `@azure/identity` and `mssql` — is unsound and was reverted. Two separate failures, both confirmed against the real Functions host, not guessed at:

1. Bundling `@azure/functions` directly fails outright: its own distributed code contains `require("util")` in a pattern esbuild's ESM output cannot resolve, and its internal `require("@azure/functions-core")` is a module the Functions worker injects at runtime and never exists in `node_modules` at all — worker load error, confirmed via `func start`.
2. Bundling it as external only postponed the same problem: `mssql` and `@azure/identity` pull in `jsonwebtoken` → `jws` → `safe-buffer`, which esbuild also cannot fully resolve, leaving broken relative-path `require()`s baked into the output (e.g. `../../node_modules/safe-buffer/index.js`). This happened to still work when run from inside the full monorepo, because that relative path coincidentally resolves to the real root `node_modules` — which is exactly why it looked fine locally and would have failed silently the moment the built `dist/` was deployed on its own.

**Fix: only first-party code is bundled.** `build.mjs` now marks every real npm dependency external (`@azure/functions`, `@azure/identity`, `mssql`, `zod` — read from `package.json`, so a new dependency is external by default without remembering to update a list) and bundles only `src/` plus the two workspace packages (`@pfc/contract`, `@pfc/scoring`), which are safe because they are plain first-party TypeScript with nothing dynamic. `dist/index.js` is now 8 KB.

**A new script, `prepare-deploy.mjs`, builds `apps/api/deploy/`** — a genuinely standalone, deployable folder: `dist/`, `host.json`, a trimmed `package.json` listing only the external dependencies, and a real `npm install` run _outside_ the workspace (in `deploy/` itself, which npm does not treat as a workspace member) so it gets a complete, correctly resolved `node_modules` rather than relying on the monorepo's hoisting. `npm run prepare-deploy -w @pfc/api` does both steps.

**Proved, not assumed: `func start` run from inside `deploy/` with zero access to the rest of the repo** served `/api/health` correctly — the same test that would have caught the earlier bug immediately, run before deploying to Azure rather than after. This is also what step 11's CI/CD deploy step should produce and upload, not the raw `apps/api` folder.
...
**Settings agreed for the Function App:** Flex Consumption, Linux, Central India, Node.js 22 LTS, 2048 MB, `func-personalfleet-api` in `Shashi-RG`, storage `stpersonafleetapi`, basic authentication disabled, continuous deployment disabled (step 11 deploys it from the existing workflow). Application Insights cannot be enabled from the create form for this combination — attach it afterwards from the Function App's Application Insights blade.

**First storage-policy block resolved (22 Sep 2026).** Built it the way the policy wanted: `vnetpersonafleet` (10.0.0.0/16) with an outbound subnet `snetpersonafleet` (10.0.0.0/24), a network-secured storage account, and the Function App with outbound VNet integration only — no inbound private endpoint, so public access stayed on. Storage account creation then succeeded.

**Second, separate block: a policy on the Function App itself.** Creating `func-personafleet` with public access on failed under a different subscription-wide assignment — _Function apps should disable public network access_ (built-in definition `969ac98b-88a8-449f-883c-2e9adb123127`, Deny). This one is a genuine conflict, not something to configure around: the policy requires `publicNetworkAccess: Disabled`; AD-20's linked-backend model requires it enabled, since the Static Web App's proxy calls the Function App over the public internet. Options: (1) a **Policy exemption** for `Shashi-RG` scoped to this assignment, same pattern as the storage exemption; (2) make the Function App fully private and connect it via a private-endpoint-aware ingress. Recommended: (1).

**Option 2 checked against Microsoft's docs (22 Sep 2026) and ruled out — a platform limit, not a configuration gap.** [Bring your own functions to Azure Static Web Apps](https://learn.microsoft.com/en-us/azure/static-web-apps/functions-bring-your-own): "An existing Functions app needs to not apply... restricting the IP address of the Functions app [or] restricting traffic through private link or service endpoints." [API support overview](https://learn.microsoft.com/en-us/azure/static-web-apps/apis-overview) states this as a blanket rule for every backend type the feature supports (Functions, API Management, App Service, Container Apps): "**Network isolated backends are not supported.**" So switching backend type would not help; a genuinely private backend would mean replacing the linked-backend model entirely with Front Door Premium (Private Link origin) or API Management Standard v2 as a VNet-integrated ingress — a few hundred dollars a month and materially more operational surface, disproportionate here. The exemption is therefore the only path that stays inside the architecture Static Web Apps was designed for.
...
**Resolved (22 Sep 2026).** Exemption granted at `Shashi-RG` scope, covering the whole policy category rather than one assignment — the broader ask paid off, since enabling the managed identity afterwards hit the same policy again on an unrelated PATCH and the group-level exemption covered that too, with no second request needed. `func-personalfleet-api` created and running: Flex Consumption, Linux, Central India, public access on, outbound-only VNet integration to the network-secured storage account.

**Network-secured storage abandoned as unworkable for Flex Consumption (22 Sep 2026).** Deployment repeatedly failed at `[Kudu-ValidationStep] [StorageAccessibleCheck]` with `BlobUploadFailedException: This request is not authorized to perform this operation`, regardless of adding the client IP or the VNet subnet to the storage firewall — because the failing caller was never the client machine (its own upload to Kudu succeeded every time); it was Kudu's own deployment pipeline reaching the storage account. Confirmed as a known Flex Consumption limitation, not a misconfiguration: deployment traffic from Kudu "does not consistently route through the VNet integration path... appears to originate from Microsoft's shared platform infrastructure rather than the VNet." A community workaround exists (user-assigned identity, three specific RBAC roles, `AzureWebJobsStorage__accountName` in place of a connection string) but is unofficial and a materially bigger change than this warrants.

**Decision: `stpersonafleet`'s public network access is set back to Enabled from all networks**, now covered by the resource-group-wide exemption obtained today. This account holds only the Function App's own deployment packages and runtime state — never Persona Fleet data — so this doesn't weaken the real access controls, which are unrelated: tenant-restricted Entra sign-in on every route, and `GRANT SELECT` scoped to exactly the ten Fabric views. Also simplifies step 11: no self-hosted-runner-in-VNet complexity needed for CI/CD deployment either, since GitHub-hosted runners would have hit this identical wall.
...
**Step 4 complete (22 Sep 2026).** `/api/health` returns `status: ok` with `fabric: connected in 6012ms, data as of 2026-09-09` — real lakehouse data, read by the managed identity through the ten views, served same-origin through the Static Web App. The check query confirms the principal exists as an `EXTERNAL_USER` with `CONNECT` plus `SELECT`.

Getting there hit one last blocker worth recording: **the Share button was greyed out**, and neither the lakehouse nor the SQL analytics endpoint offered **Manage permissions**. That is a role gap, not a navigation problem — [How lakehouse sharing works](https://learn.microsoft.com/fabric/data-engineering/lakehouse-sharing#troubleshoot-common-issues) says so directly: "You can't select Share: verify that you have share permission for the lakehouse (Admin or Member by default, or explicit Share permission)." Both items are owned by another user, and every in-portal route to grant the identity access runs through that same permission, so it had to be done by someone holding it.

Alternatives considered and rejected, since this will recur in step 5: adding the identity to a **workspace role** is the other documented way for an SPN to connect, but the minimum role that can connect is Viewer, and [workspace roles](https://learn.microsoft.com/fabric/data-warehouse/workspace-roles) grants Viewer "CONNECT and **ReadData**" — `db_datareader`-equivalent blanket read over every table in the workspace, which would make the ten grants decorative and defeat AD-3. Assigning workspace roles needs Admin anyway, so it was equally blocked. The real fallback, if the role request had stalled, was a **lakehouse we own with OneLake shortcuts** to the `persona_*` tables: shortcuts copy no data, and a T-SQL endpoint in delegated identity mode reads the target as the item owner, so the identity would need connect-only on our own item — which we could grant ourselves. More moving parts, and it breaks if OneLake security rules are ever defined on the source, so it stayed a contingency.

**6 seconds to connect** is cold-start plus token acquisition, not query time. Acceptable for a health check; step 10 should confirm it is not on the path of every user request.

**Exactly what Fabric requires, confirmed (22 Sep 2026).** [Microsoft Entra authentication as an alternative to SQL authentication](https://learn.microsoft.com/fabric/data-warehouse/entra-id-authentication#fabric-configuration) is explicit that a SQL connection by an SPN needs configuration in **both** places, and either one alone is insufficient:

1. **Tenant setting** — Fabric admin portal → Tenant settings → Developer settings → **Service principals can use Fabric APIs**, Enabled, scoped either to the whole organisation or to a security group that contains this identity. Required specifically so an SPN can "interface for SQL connection strings to Fabric warehouse or SQL analytics endpoint items". A system-assigned managed identity is an SPN for this purpose and appears in Entra under its resource name, `func-personalfleet-api`. This needs a Fabric **tenant** admin, so it may need the same route as the earlier Entra admin consent.
2. **Workspace/item setting** — either any workspace role (Admin, Member, Contributor or Viewer all suffice to connect), or, better for AD-3, item-level: the lakehouse's SQL analytics endpoint → More options → **Manage permissions** → Add user → add `func-personalfleet-api` with **no additional permissions ticked**, which the doc describes as granting "connect permissions only". That is precisely the shape AD-3 wants — the right to connect and nothing else, with all real access coming from the per-view `GRANT SELECT`. It is also why the "Read all with SQL analytics endpoint" tick must stay off (see above): it would grant blanket read and bypass the grants entirely.

Then run [sql/grants/persona_vw_api_v1_grants.sql](sql/grants/persona_vw_api_v1_grants.sql) against the ten `persona_vw_api_v1_*` views. Permission changes can take up to two hours to propagate, so a continued login failure immediately after is not yet evidence of a mistake.

**Sharing navigation corrected, and a trap avoided (22 Sep 2026).** [Lakehouse sharing and permission management](https://learn.microsoft.com/en-us/fabric/data-engineering/lakehouse-sharing): sharing is a top-level **Share** icon next to the item in the workspace list (or "..." → Share) — not nested under Settings, which only shows the connection string. The dialog offers an additional permission, **"Read all with SQL analytics endpoint"** ("Read data from the SQL analytics endpoint through T-SQL **without SQL policy**") — this must stay unchecked, since it bypasses the per-view `GRANT SELECT` entirely and grants blanket read on every table, defeating AD-3. Plain **Read** is the correct, minimum grant: it only permits connecting: fine-grained access comes from the SQL `GRANT` statements. Permission changes can take up to two hours to propagate.

**App setting names collided with a reserved pattern (22 Sep 2026).** The portal's Environment variables blade rejected `FABRIC_SQL_ENDPOINT` — underscores work everywhere else (this app's own `FUNCTIONS_WORKER_RUNTIME` has one), so this is most likely a naming pattern this subscription's AI/Fabric-integration preview features reserve for their own connection settings, not a general restriction. Renamed to PascalCase without the `FABRIC_` prefix: `FabricSqlEndpoint`, `FabricSqlDatabase` (and the optional `FabricConnectTimeoutMs`, `FabricQueryTimeoutMs`, `FabricPoolMax`), matching what the portal actually accepted. `config.ts` and every reference updated; verified end to end in the isolated `deploy/` folder — with the new names set, `isConfigured()` correctly flips to true and attempts a real connection.

**`CREATE USER` is not supported on the SQL analytics endpoint.** Confirmed against [Microsoft's docs](https://learn.microsoft.com/en-us/fabric/data-warehouse/sql-granular-permissions): "You can't explicitly run `CREATE USER` in a Fabric warehouse or SQL analytics endpoint. When you run `GRANT` or `DENY`, Fabric creates the database user automatically." [sql/grants/persona_vw_api_v1_grants.sql](sql/grants/persona_vw_api_v1_grants.sql) is `GRANT` only, with a check query. The same doc: "The user can't connect until they also have sufficient workspace-level rights" — the Share-with-Read step above is not optional, `GRANT` alone does not let the identity connect.

**Also corrected: the Function App's real name has an extra "l".** `func-personalfleet-api`, not `func-personafleet-api` used by mistake throughout earlier steps — fixed everywhere in this document, the grants script and every command since.

**`/api/health` 404: the Function App's function list was empty (22 Sep 2026).** The 404 through the Static Web App was a clean `404`/`Content-Length: 0` from the SWA proxy, not a connection failure. Ruled out in order, each disproved rather than assumed: the SWA→backend link (Backend Details showed the right app, Production, default slot); the backend's authentication settings (they did have a real fault — "Unauthenticated requests" was set to _HTTP 302 redirect_ with an invalid target, which the portal itself flagged; corrected to **HTTP 401 Unauthorized**, the right setting for an API — but the 404 persisted); unlinking and relinking the backend. The actual finding came from the Function App's own **Functions** tab: completely empty. Nothing was ever indexed, so auth and linking were never reached. `AzureWebJobsFeatureFlags=EnableWorkerIndexing` did not change it.

**What the docs actually say, and what we changed.** [Troubleshoot Node.js apps in Azure Functions (v4)](https://learn.microsoft.com/azure/azure-functions/functions-node-troubleshoot#no-functions-found) — "No functions found" is a known failure mode whose cause is normally an **error thrown while loading the entry point**, which otherwise fails silently: exactly these symptoms. The documented diagnostic is the app setting **`FUNCTIONS_NODE_BLOCK_ON_ENTRY_POINT_ERROR = true`**, "recommended for all model v4 apps", which surfaces entry-point errors in Application Insights instead of swallowing them. That setting is now part of this app's configuration permanently, not just for this investigation.

**Confirmed fixed (22 Sep 2026).** After redeploying the CommonJS build, `health` appeared in the Functions tab and `/api/health` returned a real body through the Static Web App: `contract` ok, `scoring` ok, `fabric` failing with a login error — i.e. the whole path (SWA proxy → linked backend → indexed function → handler) works, and only the Fabric grant remains. So the build format was the cause, though see the caveat below on what that does and does not prove.

Separately, the build output moved from **ESM to CommonJS** (`dist/index.js`, no `"type": "module"`). Being precise about why, because the earlier reasoning here was wrong: ES modules are **supported** by Functions and the docs describe no ESM indexing bug, so ESM was never proven to be the cause. CJS was chosen on its own merits — with every real npm dependency external, CJS output needs no `require`-shim interop for them at all, which is the exact mechanism that produced the broken bundled requires above; and plain `.js` with no `"type"` field is the form every v4 doc example uses for `main`, so it is the best-trodden path for the worker's entry-point resolution. (`.cjs` was tried first and rejected — it forces the interpretation unambiguously, but is undocumented for `main` resolution, which risks trading one silent failure for another.) Re-verified in the isolated `deploy/` folder: `health: [GET] /api/health` listed and executing. Local `func start` passed under the ESM build too, so a local pass alone does not settle this — the Azure deploy does.
...
**Status:** Done, 24 Sep 2026. `Persona_Config_Dev` created, schema and seed applied, all six check blocks pass. Scripts: [sql/config/05a_schema.sql](sql/config/05a_schema.sql), [sql/config/05b_seed_persona_policy.sql](sql/config/05b_seed_persona_policy.sql), [sql/config/05c_grants.sql](sql/config/05c_grants.sql), [sql/checks/05_config_store.sql](sql/checks/05_config_store.sql).
...
- **Weights must total 100** (`CK_persona_policy_weights_total`). This is the one Baselines-page edit that could corrupt every score in the portal without raising an error — pillar weights combine into persona health, so 95 or 105 silently rescales everything. A constraint rejects the write; a convention lets it through.
- **`State` matches the `ExceptionState` union exactly** (`'Pending' | 'Approved' | 'Rejected'`), so a value that would reach the front end and match no branch cannot be stored.
- **A decision needs a decider**: an exception cannot read `Approved` with a null `DecidedBy`, which is the one thing an approval record exists to prove.
- **Idempotency keys are `NOT NULL UNIQUE`**, making a retried POST record once. `NOT NULL` matters because SQL Server allows a single NULL in a unique index, so nullable keys would fail the second key-less retry for a confusing, unrelated reason.

**`RequestNumber` is generated by the database**, as `REQ` plus a zero-padded `IDENTITY`. The number is user-facing and must be unique, and two Function App instances handling concurrent requests cannot agree on the next one without a round trip — `IDENTITY` already solves that.

**No foreign keys to `persona_dimpersona`**: it lives in the lakehouse, a different engine. Nothing fails until a persona reaches the portal with no baseline to score against, so block 6 of the check compares the two stores explicitly, by three-part name from the lakehouse endpoint into the mirrored SQL database.

**The seed never overwrites.** `05b` is `INSERT ... WHERE NOT EXISTS` throughout, so re-running restores missing personas and leaves existing rows alone. `persona_policy` is the one table humans edit; a `MERGE` or `DELETE`+`INSERT` would quietly revert their work the next time anyone re-ran the file. Correcting an adopted baseline is therefore a deliberate `UPDATE` recorded here, which is what `UpdatedBy` exists to capture.

**The check exercises the constraint rather than trusting it.** Block 4 attempts an insert with weights totalling 99 inside a transaction and expects the failure, then rolls back. A constraint that is never fired is an assumption; block 3 also fails the check if any constraint is present but `is_not_trusted`, which would mean it had been bypassed and no longer proves anything.
