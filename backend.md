...
**Deployment packaging fixed (22 Sep 2026), once `func-personafleet-api` existed and had no code.** The first bundling approach — esbuild bundling everything, including `@azure/functions`, `@azure/identity` and `mssql` — is unsound and was reverted. Two separate failures, both confirmed against the real Functions host, not guessed at:
...
**Settings agreed for the Function App:** Flex Consumption, Linux, Central India, Node.js 22 LTS, 2048 MB, `func-personafleet-api` in `Shashi-RG`, storage `stpersonafleetapi`, basic authentication disabled, continuous deployment disabled (step 11 deploys it from the existing workflow). Application Insights cannot be enabled from the create form for this combination — attach it afterwards from the Function App's Application Insights blade.
